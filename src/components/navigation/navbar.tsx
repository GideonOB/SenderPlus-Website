...
export function Navbar() {
  return <header className="sticky top-0 z-40 border-b border-ink/10 bg-canvas/95 backdrop-blur-sm">
    <Container className="relative flex h-20 items-center justify-between">
      ...
      <div className="hidden items-center gap-3 lg:flex"><Link href="/track" className="px-3 py-2 text-sm font-bold">Track</Link><Link href="/send" className="rounded-md bg-ink px-5 py-3 text-sm font-bold text-white transition-colors hover:bg-sender-red">Send a Package</Link></div>
      <MobileNavigation />
    </Container>
  </header>;
}
