...
type ButtonProps = { children: ReactNode; href: string; variant?: "primary" | "secondary" | "light"; className?: string; external?: boolean };

export function Button({ children, href, variant = "primary", className = "", external = false }: ButtonProps) {
  const styles = {
    primary: "bg-ink text-white hover:bg-sender-red",
    secondary: "border border-ink/25 text-ink hover:border-ink hover:bg-white",
    light: "bg-white text-ink hover:bg-sender-blue",
  }[variant];
